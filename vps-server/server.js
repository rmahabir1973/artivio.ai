...
if (!OPENAI_API_KEY) {
  console.warn('⚠️  OPENAI_API_KEY not set - Whisper transcription disabled');
}
...
console.log('    • xfade transitions (30+ types)');
...
function buildAdvancedFFmpegCommand(clips, enhancements, videoSettings, audioAssets, multiTrackTimeline, outputPath, jobId) {
  ...
  if (videoSettings?.resolution) {
    ...
    if (resMap[videoSettings.resolution]) {
      // Scale filter applied in filter_complex instead
    }
  }
...
function buildFilterGraph(clips, enhancements, videoSettings, audioAssets, inputMap, jobId) {
  ...
  for (let i = 0; i < clips.length; i++) {
    ...
    if (clip.hasAudio && !clip.muted && !clip.isImage) {
      ...
      // Generate silent audio for this clip (clamp to minimum 0.1s)
  ...
  // Calculate total duration for mixing
  let totalDuration = 0;
  clips.forEach(clip => {
    const speed = clip.speed ?? 1;
    const displayDuration = clip.isImage ? (clip.displayDuration ?? 5) : null;
    ...
    // Clamp each clip duration to prevent negative/zero values
    dur = Math.max(dur / speed, 0.1);
    totalDuration += dur;
  });
  
  // Ensure minimum duration to prevent FFmpeg errors
  const safeDuration = Math.max(totalDuration, 0.1);
  
  // Safety check: ensure we have audio streams (fallback to silent audio)
  ...
  if (clips.length > 1) {
    if (hasTransitions) {
      ...
      for (let i = 0; i < clips.length - 1; i++) {
        ...
        // Calculate clip duration after effects
        const speed = clip.speed ?? 1;
        const displayDuration = clip.isImage ? (clip.displayDuration ?? 5) : null;
        ...
        clipDur = Math.max(clipDur / speed, 0.1); // Clamp to prevent zero duration
      ...
      // Audio handling: Only concat if audioStreams matches clip count
      // Otherwise use the fallback directly (no concat needed for single silent source)
      if (audioStreams.length === clips.length) {
        ...
        // All clips silent - use fallback directly without concat
        finalAudioLabel = earlyAudioFallbackLabel;
      } else if (audioStreams.length === 1) {
        // Only one audio source but multiple clips - use it directly
        finalAudioLabel = audioStreams[0];
      } else {
        // No audio at all - will be caught by later validation
        finalAudioLabel = '';
      }
    ...
    // Single clip case - use first audio stream or fallback
  ...
  // Ensure finalAudioLabel is valid (fallback to silent if still empty)
  ...
  // Ensure totalDuration is at least 0.1 to avoid zero-length FFmpeg errors
  totalDuration = Math.max(totalDuration, 0.1);
  
  // Mix audio sources
  const audioSources = [];
  
  // Only add finalAudioLabel if it's valid (not empty)
  if (finalAudioLabel && finalAudioLabel.trim() !== '' && finalAudioLabel !== '[]') {
    audioSources.push(finalAudioLabel);
  } else {
    // Generate silent audio as fallback and update finalAudioLabel
  ...
  if (audioAssets.backgroundMusic) {
    ...
    // Trim to video length and loop if needed
  ...
  // Ensure video has final label
  if (!finalVideoLabel.startsWith('[') || !finalVideoLabel.endsWith(']')) {
    finalVideoLabel = `[${finalVideoLabel.replace(/[\[\]]/g, '')}]`;
  }
  
  // FINAL SAFETY: Guarantee finalAudioLabel is never empty before output
  // This is the last line of defense for silent/muted timelines
  if (!finalAudioLabel || finalAudioLabel.trim() === '' || finalAudioLabel === '[]') {
    // Use duration from totalDuration which was already clamped
    ...
    console.log(`[${jobId}] Warning: Applied final audio fallback - all clips were silent`);
  }
...
function getWatermarkPosition(position, width, height, size) {
  const padding = 20;
  const wmWidth = size === 'small' ? width * 0.1 : size === 'large' ? width * 0.25 : width * 0.15;
  const wmHeight = wmWidth * 0.5; // Approximate
...
function executeFFmpeg(args, jobId, jobsMap) {
  return new Promise((resolve, reject) => {
    console.log(`[${jobId}] FFmpeg args:`, args.slice(0, 20).join(' ') + '...');
