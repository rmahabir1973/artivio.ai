...
-   **Background Remover**: AI-powered background removal using Recraft's remove-background model. Allows users to upload PNG, JPG, or WebP images (max 5MB) and automatically removes the background. Costs 3 credits per use (1 Kie.ai credit). Features file validation, credit preview, and background task processing with webhook callbacks for real-time status updates. Integrated under new "Image" sidebar menu.
