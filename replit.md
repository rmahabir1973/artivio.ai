...
-   **Content Pipeline Quick Actions**: Library generation cards feature quick action buttons enabling one-click transformations. Actions are context-aware based on content type: images can "Create Video" (navigate to AI Video with image pre-loaded), "Remove Background", "Send to Social"; videos can "Add to Video Editor", "Send to Social". Uses sessionStorage to pass asset URLs between pages. SessionStorage keys: `quickAction_imageToVideo`, `quickAction_bgRemove`, `quickAction_socialAsset`, `videoEditor_clips`. Backend background-remover uses `processImageInputs()` to handle both base64 and URL inputs. Social calendar shows library asset preview with "Content from Library" section.
